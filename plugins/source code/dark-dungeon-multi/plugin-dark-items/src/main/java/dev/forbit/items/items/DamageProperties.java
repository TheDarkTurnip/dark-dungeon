...
public class DamageProperties {
    ...
    public DamageProperties(Weapon item) {
        ...
        if (item instanceof Modifiable) {
            ...
            this.modifierMultiple = (modifiable.getModifier() != null) ? modifiable
                    .getModifier()
                    .getDamageModifier() : 1.0;
        }
    ...
    private double getSharpnessDamage() {
        if (getItem() instanceof Enchantable) {
            ...
            double sharpnessLevel =
                    enchantable.getEnchantment(Enchantment.SHARPNESS) + enchantable.getEnchantment(Enchantment.POWER) + enchantable
                            .getEnchantment(Enchantment.STRENGTH);
    ...
    public List<String> getDamageString() {
        ...
        if (this.getModifierDamage() > 0) list.add(ChatColor.GRAY + "Modifier Damage: " + ChatColor.WHITE + Utils.round(
                this.getModifierDamage()));
        if (this.getSharpnessDamage() > 0)
            list.add(ChatColor.GRAY + "Sharpness Damage: " + ChatColor.WHITE + Utils.round(this.getSharpnessDamage()));
        if (this.getRepairDamage() > 0)
            list.add(ChatColor.GRAY + "Repair Damage: " + ChatColor.WHITE + Utils.round(this.getRepairDamage()));
        if (this.getLevelsDamage() > 0)
            list.add(ChatColor.GRAY + "Level Damage: " + ChatColor.WHITE + Utils.round(this.getLevelsDamage()));
